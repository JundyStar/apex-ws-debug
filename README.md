...
- 保留最近 20 条批量业务消息的逐账号/设备投递明细，可区分已收到、在线未收到、当时未连接和后续收到，并支持筛选、搜索及复制；
- 记录单连接以及批量账号的首次连接时间，批量明细同时展示最近连接时间，方便判断消息到达时连接是否已经建立；
- 批量模式默认展示投递诊断，原始日志通过 Tab 切换；投递指标可直接点击筛选对应账号；
- 环境配置可折叠，并在首次成功连接后自动收起，以减少页面占用；
- WS Token 换新期间保留旧连接继续在线和发送心跳，页面单独显示“Token 换新中”，避免将无缝换新误判为掉线；
- 批量 WS Token 的提前换新时间分散在 30～90 秒窗口内，降低大量连接同时换新的请求峰值；
- 页面采用左窄右宽布局，为投递诊断和逐账号明细保留更多展示空间；
...
批量投递明细以每条连接触发浏览器 `WebSocket.onmessage` 为“已收到”的判断依据，不会额外发送 ACK。收到第一份消息时会快照当时在线的账号，因此即使批量连接仍在启动，也能区分“在线未收到”和“当时未连接”；明细只保存在当前页面内存中，刷新页面后清空。
